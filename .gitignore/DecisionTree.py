...
def Generate_decision_tree(D, attribute_list, depth):
    ...
    if attribute_list == []:
        N = [0, majority_wins_label(D)]
        return N
    splitting_att_full = Attribute_selection_method(D,attribute_list)
    ...
    for i in xrange(0, len(splitting_att_full[2])):
        #if number of data points in D_j is <=50, use majority wins
...
def build_confusion_matrix():
    ...
    for row in xrange(0,len(test_dataset)):
        ...
        predicted_label = predict(D_T, test_dataset[row])
...
def build_confusion_matrix_train():
    ...
    for row in xrange(0,len(dataset)):
        ...
        predicted_label = predict(D_T, dataset[row])
        conf_mat[actual_label-1][predicted_label-1] += 1
    return conf_mat
...
#build confusion matrix for testing
confusion_mat = build_confusion_matrix()
#build confusion matrix for training
...
def class_F_one(matrix):
    ...
    if precision == "divide by zero" or recall == "divide by zero":
        return "divide by zero"
...
def class_F_half(matrix):
    ...
    if precision == "divide by zero" or recall == "divide by zero":
        ...
        return 1.0*(1+.25)*precision*recall/((.25)*precision + recall)
...
def class_F_two(matrix):
    ...
    if precision == "divide by zero" or recall == "divide by zero":
        ...
        return 1.0*(1+4.0)*precision*recall/((4.0)*precision + recall)
